...
public class ExcelXlsxReader extends DefaultHandler {
     ...
     * @throws Exception
     */
    public Map<String,List<JSONObject>> process(File file) throws Exception {
        ...
        return sheetMap; //返回该excel文件的总行数，不包括首列和空行
    }
...
//                    ExcelUtil.sendRows(filePath, sheetName, sheetIndex, curRow, cellList);
                    JSONObject jsobj=JSONObject.parseObject("{}");
                    if(curRow==0){
                        objName =new String [cellList.size()] ;
                        int i=0;
...
//                        String asd="";
//                        for(String z : objName){
//                            asd+=z;
//                        }
//                        System.out.println(sheetName+""+asd);
                    }else {
                        for (int j = 0; j < cellList.size(); j++) {//循环列数
                            String value=cellList.get(j);
